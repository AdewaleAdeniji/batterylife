...
window.onload = function(){
    ...
    function startPlay(){
     ...
     levelCnt.style.display = "block";
       ...
       if("played" in localStorage){
        var pa = localStorage.getItem("played");
       }
       else {
        var pa = 0;
       }
            ...
            if(wordPos > -1){
                ...
                if(temp1.indexOf(currentWord) < 0){
                    ...
                    if(temp1.length >= levels[currentLevel][0].length){
                        ...
                        temp2 = [];
                        setTimeout(function(){
                       localStorage.setItem("played",currentLevel+1);
                       startPlay();
                        levelComplete();
                        },2000)
    ...
    mybtn.addEventListener("click",talk);
    function talk(){ 
        alert("Copyrights Reserved,This App is developed by devFeranmi....(Twitter@devferanmi)");
    }
    var coinElm = _s("coin");
